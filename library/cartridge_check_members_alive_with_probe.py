...
def probe_servers(control_console):
...
def check_members_alive_with_probe(params):
    console_sock = params['console_sock']
    allowed_states = params['allowed_states']

    control_console = helpers.get_control_console(console_sock)

    _, err = probe_servers(control_console)
    if err is not None:
...
if __name__ == '__main__':
    helpers.execute_module(argument_spec, check_members_alive_with_probe)
